...
interface Emprendimiento {
  id: number | string; // acepta number o string
  nombre: string;
  descripcion: string;
  imagen: string;
  ubicacion: string;
...
export default function ListaEmprendimientos({
  emprendimientosFiltrados,
}: ListaEmprendimientosProps) {
  return (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Conoce los negocios locales que están transformando nuestra
            comunidad con innovación y pasión.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {emprendimientosFiltrados.map((emprendimiento) => (
            <CardEmprendimiento
              key={String(emprendimiento.id)} // lo convertimos a string para evitar warning
